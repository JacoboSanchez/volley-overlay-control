...
├── pyproject.toml             # Runtime + dev dependency ranges; ruff, mypy, coverage configuration
├── uv.lock                    # Pinned, hashed lockfile for both groups (generated by `uv lock`)
...
| Surface | Gates |
|---------|-------|
| Backend | `pytest` (coverage floor 70%) · `ruff` · `mypy` · `bandit` · `pip-audit` (both dependency groups) · `uv.lock`-matches-`pyproject.toml` |
