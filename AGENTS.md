...
- **Auth / accounts:** `SESSION_SECRET` (auto-minted and persisted to `data/.session_secret` if unset; also signs match-report capability URLs unless `MATCH_REPORT_SECRET` is set — pin it explicitly to keep sessions/links valid across deployments). `REGISTRATION_OPEN` (seeds the DB toggle for self-registration; admins flip it at runtime). First-admin claim uses a one-time token logged at startup (see Auth, below) — there is no admin password env var.
- **Metrics:** `/metrics` is unauthenticated by default (aggregates only — no payloads, no per-OID labels); `METRICS_TOKEN` gates it behind a bearer token and `METRICS_ENABLED=false` removes it. See [AUTHENTICATION.md](AUTHENTICATION.md) §10.
- **Observability:** unhandled exceptions are logged with request context and handed to `app/observability.py`, the single hook for an optional error reporter (`SENTRY_DSN`; `sentry-sdk` is an optional import, not a bundled dependency). A valid inbound W3C `traceparent` is adopted as the request id and logged as `trace_id` (`app/api/middleware/logging.py`).
- **Never put a raw request path on a log record.** Use `logging_utils.redact_path()`. `/overlay/<public_token>`, `/follow/…`, `/ws/…`, `/matches/…` and `/match/<match_id>/report` carry a live capability token in the path. The `redact` filter is registered on the stdout/file **handlers**, so it does not protect a record from anything that installs its own handler — Sentry's logging integration promotes `ERROR` records to events, message and `extra` included. `observability._scrub_event` is the backstop (it sweeps `request`, `logentry`, `extra`, `breadcrumbs`, `transaction`), but do not rely on it alone. Note `send_default_pii=False` does **not** strip the URL, query string or body.
- **Remote config:** `EnvVarsManager` is stale-while-revalidate — a failed fetch **retains** the previous values instead of clearing them. Never "reset to empty" on failure: a security-relevant value read through it (e.g. `METRICS_TOKEN`) would silently revert to unset and fail open.
