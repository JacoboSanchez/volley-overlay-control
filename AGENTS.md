...
- **Visual-only settings must use browser-local storage** — any setting that has no effect on overlay output must be listed in `_LOCAL_STORAGE_KEYS` in `app_storage.py`. `AppStorage.save()`/`load()` dispatch to `app.storage.browser` for those keys automatically, keeping them isolated per-browser and preventing them from propagating to other connected clients. Current local-only settings: button colors, button font, button icon visibility/opacity.
