...
class ScoreButton:
    ...
    def create(gui, team_id, is_set_button, text, color, classes_str=None):
        ...
        button.on('mousedown', lambda: gui.handle_button_press(team_id, is_set_button=is_set_button))
        button.on('touchstart', lambda: gui.handle_button_press(team_id, is_set_button=is_set_button), [])
