...
overlay_state_store = OverlayStateStore(data_dir=_data_dir, templates_dir=_templates_dir)
obs_broadcast_hub = ObsBroadcastHub()


def _on_state_changed(overlay_id: str) -> None:
    """Callback wired into the state store to trigger OBS broadcasts."""
    obs_broadcast_hub.schedule_broadcast_from_sync(
        overlay_id,
        lambda oid=overlay_id: overlay_state_store.get_state(oid),
    )
