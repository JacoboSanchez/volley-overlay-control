...
# Conservative whitelist for the username/oid embedded in a per-board manifest's
# ``start_url`` — matches the overlay-id rules (letters, digits, ``. _ -``).
...
def _board_manifest(base: dict, title: str, u: str | None, oid: str) -> dict:
    ...
    manifest["id"] = start
    manifest["scope"] = "/"
    manifest["name"] = f"{title} · {oid}"
    manifest["short_name"] = oid
    # Boards share one icon, distinct from the base app's. Relative ``src`` is
    # resolved against the manifest URL's path (the query string is ignored), so
    # these land on the static ``/icon-board.*`` files. The raster siblings are
    # regenerated by scripts/regenerate-icons.sh; until then Chrome falls back
    # to the SVG entry (a missing PNG is simply skipped).
    manifest["icons"] = [
        {"src": "icon-board.svg", "sizes": "any", "type": "image/svg+xml",
         "purpose": "any maskable"},
        {"src": "icon-board-192x192.png", "sizes": "192x192", "type": "image/png",
         "purpose": "any maskable"},
