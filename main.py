    ...
    def init(self, force=True):
        ...
        with ui.row().classes("w-full justify-right"):
            ...
            with ui.element('q-fab').props('icon=settings_suggest color=sky-800 direction=left').classes('text-white'):
                ui.button(icon='settings', color='blue-500', on_click=lambda: tabs.set_value(CONFIG_TAB)).props('round').classes('text-white')    
...
scoreboard = ui.tab("Scoreboard")
configuration = ui.tab(CONFIG_TAB)
gui = GUI(tabs)
...
@ui.page("/")
def main():
    with ui.tab_panels(tabs, value=scoreboard).classes("w-full"):
        with ui.tab_panel(scoreboard):
            gui.init()
        with ui.tab_panel(configuration):
            customization_page = CustomizationPage(tabs)
