...
export default function AccountHome() {
  ...
  return (
    <div>
      ...
      {error && (
        <div className="acc-error" style={{ marginTop: 16 }}>
          {t('acc.reports.errorOverlays')}
        </div>
      )}
      ...
      <div className="acc-tile-grid">
        <Tile
          to="/overlays"
          title={t('acc.nav.overlays')}
          desc={t('acc.home.tile.overlays.desc')}
        />
        ...
        {isAdmin && (
          <Tile to="/admin" title={t('acc.nav.admin')} desc={t('acc.home.tile.admin.desc')} />
        )}
      </div>
    </div>
  );
