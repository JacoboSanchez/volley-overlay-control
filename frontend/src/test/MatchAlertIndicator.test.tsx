...
function withInfo(overrides: Partial<GameState>, info: Partial<typeof baseInfo> = {}): GameState {
  return {
    ...mockGameState,
...
describe('MatchAlertIndicator', () => {
  ...
  it('renders the match-finished pill when match_finished is true', () => {
    renderWithI18n(<MatchAlertIndicator state={withInfo({ match_finished: true })} />);
  ...
  it('match_finished overrides set/match-point flags', () => {
    ...
    expect(screen.getByTestId('match-alert-indicator').dataset.alertKind).toBe('finished');
  });

  it('renders set-point team 1 when only team 1 has set point', () => {
    renderWithI18n(<MatchAlertIndicator state={withInfo({}, { team_1_set_point: true })} />);
    const el = screen.getByTestId('match-alert-indicator');
  ...
  it('prefers match-point over set-point when both are true', () => {
    renderWithI18n(
      <MatchAlertIndicator
        state={withInfo({}, { team_2_set_point: true, team_2_match_point: true })}
      />,
    );
    const el = screen.getByTestId('match-alert-indicator');
  ...
  it('renders set-point for team 2 when only team 2 has set point', () => {
    renderWithI18n(<MatchAlertIndicator state={withInfo({}, { team_2_set_point: true })} />);
    const el = screen.getByTestId('match-alert-indicator');
    expect(el.dataset.alertKind).toBe('set-point');
    expect(el.dataset.alertTeam).toBe('2');
  });
  ...
  it('places the icon before the label for team 1', () => {
    renderWithI18n(<MatchAlertIndicator state={withInfo({}, { team_1_match_point: true })} />);
    const el = screen.getByTestId('match-alert-indicator');
  ...
  it('places the icon after the label for team 2', () => {
    renderWithI18n(<MatchAlertIndicator state={withInfo({}, { team_2_match_point: true })} />);
    const el = screen.getByTestId('match-alert-indicator');
    const children = Array.from(el.children) as HTMLElement[];
    const iconIdx = children.findIndex((c) => c.classList.contains('material-icons'));
    const labelIdx = children.findIndex((c) => !c.classList.contains('material-icons'));
  ...
  it('uses a left-pointing triangle for team 1 in landscape', () => {
    renderWithI18n(
      <MatchAlertIndicator state={withInfo({}, { team_1_set_point: true })} isPortrait={false} />,
    );
  ...
  it('uses a right-pointing triangle for team 2 in landscape', () => {
    renderWithI18n(
      <MatchAlertIndicator state={withInfo({}, { team_2_match_point: true })} isPortrait={false} />,
    );
    const icon = screen.getByTestId('match-alert-indicator').querySelector('.material-icons');
  ...
  it('uses an up-pointing triangle for team 1 in portrait', () => {
    renderWithI18n(
      <MatchAlertIndicator state={withInfo({}, { team_1_set_point: true })} isPortrait={true} />,
    );
    const icon = screen.getByTestId('match-alert-indicator').querySelector('.material-icons');
  ...
  it('uses a down-pointing triangle for team 2 in portrait', () => {
    renderWithI18n(
      <MatchAlertIndicator state={withInfo({}, { team_2_match_point: true })} isPortrait={true} />,
    );
    const icon = screen.getByTestId('match-alert-indicator').querySelector('.material-icons');
  ...
  it('keeps the trophy icon for match-finished regardless of orientation', () => {
    for (const isPortrait of [true, false]) {
      const { unmount } = renderWithI18n(
        <MatchAlertIndicator state={withInfo({ match_finished: true })} isPortrait={isPortrait} />,
      );
      const icon = screen.getByTestId('match-alert-indicator').querySelector('.material-icons');
