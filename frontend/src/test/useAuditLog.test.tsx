...
describe('useAuditLog', () => {
  ...
  it('does not fetch while disabled', async () => {
    renderHook(() =>
      useAuditLog('test-oid', false, { trigger: mockGameState }),
    );
  ...
  it('fetches when enabled and surfaces the page', async () => {
    vi.mocked(api.getAudit).mockResolvedValue({
      ...
      count: 2,
      records: [
        record(1, 'add_point', { team: 1 }),
        record(2, 'add_point', { team: 2 }),
      ],
    });
    ...
    expect(api.getAudit).toHaveBeenCalledWith(
      'test-oid', 10, expect.any(AbortSignal),
    );
  ...
  it('refetches when the trigger key changes', async () => {
    const { rerender, result } = renderHook(
      ({ s }: { s: GameState }) =>
        useAuditLog('test-oid', true, { trigger: s }),
  ...
  it('records error message on fetch failure', async () => {
    ...
    const { result } = renderHook(() =>
      useAuditLog('test-oid', true, { trigger: mockGameState }),
    );
    await waitFor(() => expect(result.current.loading).toBe(false));
  ...
  it('clears records and stops fetching when oid becomes null', async () => {
    ...
    const { rerender, result } = renderHook(
      ({ id }: { id: string | null }) =>
        useAuditLog(id, true, { trigger: mockGameState }),
