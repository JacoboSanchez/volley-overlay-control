...
const svgs = readdirSync(dir).filter(f => /^1[3-8]-.*\.svg$/.test(f)).sort();
...
const sheet = `<!doctype html><html><body style="margin:0;background:#f0f1f5;padding:40px">
  <div style="font:800 26px system-ui;color:#111;margin-bottom:6px">Icon concepts — batch 3</div>
  <div style="font:500 15px system-ui;color:#666;margin-bottom:28px">More directions, shown at 180px with rounded-app mask</div>
...
await body.screenshot({ path: path.join(dir, 'contact-sheet-3.png') });
console.log('rendered contact-sheet-3.png');
await browser.close();
