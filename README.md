...
*   **Points History Strip**: When the overlay preview is hidden, the centre column shows a two-row action timeline (one row per team) sourced from the audit log. Each rally, set win, match win, timeout and manual correction renders as a chip on its team's row only — `+1`/`−1` for points and undos, a star when a set is won and a trophy when the match ends, a clock for timeouts (with the forward chip and the strike-through chip both collapsing when the undo is immediate, and the original being reconstructed from the post-state diff when there were actions in between), and a pencil with the new score for manual corrections. Logo + colour markers anchor each row; honours `followTeamColors` and per-team customisation overrides. Window auto-shrinks on narrow landscape phones so it never overflows the centre slot.
