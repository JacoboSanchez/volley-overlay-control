...
| `SENTRY_DSN` | *(Optional)* Enables error-tracking. Unhandled exceptions are reported (tagged with `request_id` and, when an upstream sent a `traceparent`, `trace_id`) in addition to being logged. Outgoing events are scrubbed of capability tokens, query strings, request bodies, cookies and credential headers before they leave the process. Requires the `sentry-sdk` package, which is **not bundled** — add it to your image. A DSN set without the package logs a warning and leaves reporting off. | *(unset → logging only)* |
