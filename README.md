...
It includes 19 overlay style templates served directly to OBS browser sources — no external overlay server required. It also works with *overlays.uno* cloud overlays and with fully custom, external overlay engines. Complete match control — scores, sets, timeouts, and serving teams. Highly customizable and built for versatility, it supports multiple users, overlays, and personalized themes.
...
**Built-in overlay styles** rendered live to OBS browser sources via `/overlay/{id}`. All 19 selectable styles laid out side-by-side in a single preview grid via `/overlay/{id}?style=mosaic` (the simple-mode grid below is captured with the forced light theme — `?theme=light`):
