...
*   **Safe Multi-Operator Control**: Every state carries a monotonic revision, so the bundled UI rejects and reconciles stale concurrent writes instead of overwriting another operator's newer action. A quiet aggregate indicator appears when multiple control tabs are connected; ephemeral tab attribution never exposes the owner's account identity.
...
Planned usability, functionality, and performance work is tracked in the
[improvement roadmap](docs/USABILITY_FUNCTIONALITY_PERFORMANCE_ROADMAP.md).
...
*   **Import/export** — On the Team catalog page, admins can download a
    versioned JSON backup and choose whether to embed hosted logos. Import
    previews name conflicts and offers **Replace**, **Save with another name**,
...
| `/api/v1/matches[?oid=X]` | `GET` — list the user's archived match reports with owner-scoped OID/mode/date filters, stable sorting and `limit`/`offset` paging. |
| `/api/v1/matches/days` | `GET` — distinct local calendar days containing the user's archived matches (optional OID/mode filters). |
| `/api/v1/matches/bulk-delete` | `POST` — transactionally delete up to 100 selected reports owned by the caller. |
