...
*   **Points History Strip**: When the overlay preview is hidden, the centre column shows up to the last five rallies as team-coloured chips (oldest on the left) sourced from the audit log, so operators can read recent momentum at a glance without bringing the preview iframe back. Honours `followTeamColors` and per-team colour overrides.
