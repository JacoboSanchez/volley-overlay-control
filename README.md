...
**Built-in overlay styles** rendered live to OBS browser sources via `/overlay/{id}`. All 19 selectable styles laid out side-by-side in a single preview grid via `/overlay/{id}?style=mosaic`. The pair below doubles as the theme demo — full data is captured with the forced light theme (`?theme=light`), simple mode with the forced dark one (`?theme=dark`); styles without the matching palette keep their native look:
