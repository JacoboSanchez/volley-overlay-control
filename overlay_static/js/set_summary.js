...
(function () {
  ...
  const LABELS = {
    en: {
      ...
      statsGeneral: "Overall",
      statsPointTypes: "Point types",
    ...
    es: {
      ...
      servicesWon: "Servicios ganados",
      timeoutsUsed: "Tiempos muertos",
      home: "Local",
      away: "Visitante",
      progression: "Progresión de puntos",
      recap: "Resumen del set",
      streak: "Racha",
      services: "Servicios",
      timeouts: "Tiempos",
      points: "Puntos",
      bestOf: "Mejor de",
      setWinner: "Ganador del set",
      runnerUp: "Segundo",
      live: "EN VIVO",
      vs: "VS",
      pointsShort: "pts",
      empty: "Aún sin puntos en este set",
      statsGeneral: "Generales",
      statsPointTypes: "Tipos de punto",
    ...
    pt: {
      ...
      servicesWon: "Serviços ganhos",
      timeoutsUsed: "Tempos pedidos",
      home: "Casa",
      away: "Visitante",
      progression: "Progressão de pontos",
      recap: "Resumo do set",
      streak: "Sequência",
      services: "Serviços",
      timeouts: "Tempos",
      ...
      statsGeneral: "Gerais",
      statsPointTypes: "Tipos de ponto",
    ...
    it: {
      ...
      servicesWon: "Servizi vinti",
      timeoutsUsed: "Timeout usati",
      home: "Casa",
      away: "Ospiti",
      progression: "Progressione punti",
      recap: "Riepilogo set",
      streak: "Striscia",
      services: "Servizi",
      timeouts: "Timeout",
      ...
      statsGeneral: "Generali",
      statsPointTypes: "Tipi di punto",
    ...
    fr: {
      ...
      servicesWon: "Services gagnés",
      timeoutsUsed: "Temps morts",
      home: "Domicile",
      away: "Visiteur",
      progression: "Progression des points",
      recap: "Récap. du set",
      streak: "Série",
      services: "Services",
      ...
      statsPointTypes: "Types de point",
    ...
    de: {
      ...
      servicesWon: "Aufschläge gewonnen",
      timeoutsUsed: "Auszeiten",
      home: "Heim",
      away: "Auswärts",
      progression: "Punktverlauf",
      recap: "Satzrückblick",
      streak: "Serie",
      services: "Aufschläge",
      timeouts: "Auszeiten",
      ...
      statsGeneral: "Gesamt",
      statsPointTypes: "Punktarten",
      pointDiff: "Punktdifferenz",
      setPoint: "Satzball",
      chipAce: "Ass",
      chipOppErr: "Geg.-F.",
    },
  ...
  function teamLogoNode(team, side, className) {
    ...
    if (team.logo_url) {
      const img = el("img", {
        attrs: { src: team.logo_url, alt: team.name || side },
      });
  ...
  function deriveViewModel(state) {
    ...
    const setLongest =
      longestBySet[setNum] || longestBySet[String(setNum)] || {};
    const longestSet = {
      1: Number(
        (setLongest["1"] != null ? setLongest["1"] : setLongest[1]) || 0,
      ),
      2: Number(
        (setLongest["2"] != null ? setLongest["2"] : setLongest[2]) || 0,
      ),
    };
    ...
    const setPtRaw =
      pointTypesBySet[setNum] || pointTypesBySet[String(setNum)] || {};
    ...
    const durationSec =
      setDurations[setNum] || setDurations[String(setNum)] || 0;
    ...
    return {
      ...
      matchElapsedSec,
      matchStartedAt:
        typeof matchStartedAt === "number" ? matchStartedAt : null,
      matchFinishedAt:
        typeof matchFinishedAt === "number" ? matchFinishedAt : null,
      home,
      away,
      homeScore,
      awayScore,
  ...
  function ensurePanel() {
    ...
    panel = el("div", {
      class: "set-summary-panel",
      attrs: { id: "set-summary-panel" },
    });
  ...
  function buildStat(label, value) {
    return el("div", {
      ...
      children: [
        ...
        el("span", {
          class: "ss-value",
          text: value === 0 || value === "0" ? "–" : String(value),
        }),
      ],
    });
  }
      ...
      children: [
        ...
        el("div", {
          ...
          children: [
            ...
            matchClockNode(vm.matchElapsedSec, {
              class: "ss-value ss-match-duration",
            }),
          ],
        }),
      ],
  ...
  function buildChartWrap(vm) {
    const wrap = el("div", { class: "ss-chart-wrap" });
    wrap.appendChild(
      buildSvgChart(vm, { width: 300, height: 380, padLeft: 6, padRight: 6 }),
    );
  ...
  function buildSvgChart(vm, opts) {
    ...
    svg.appendChild(
      svgEl("polyline", { class: "ss-line-home", points: homePts }),
    );
  ...
  function renderGlass(stage, vm) {
            ...
            children: [
              ...
              el("span", {
                ...
                children: [
                  el("span", { class: "ss-clock-label", text: t("match") }),
                  matchClockNode(vm.matchElapsedSec, {
                    class: "ss-clock ss-match-duration",
                  }),
                ],
              }),
            ],
    ...
    box.appendChild(
      buildSvgChart(vm, { width: 480, height: 360, padTop: 8, padBottom: 6 }),
    );
  ...
  function buildGlassStatRowDual(label, homeVal, awayVal) {
    ...
    return el("div", {
      class: "ss-stat-row",
      children: [el("span", { class: "ss-label", text: label }), value],
    });
  }
  ...
  function buildLdGeneralCol(vm) {
    ...
    col.appendChild(
      buildLdRow(t("timeouts"), toH, toA, toH || 0.4, toA || 0.4),
    );
    return col;
  }
  ...
  function buildLdDiffChart(vm) {
    ...
    if (total > 0) {
      ...
      svg.appendChild(
        svgEl("polyline", { class: "ss-ld-line", points: lineStr }),
      );
    }
    svg.appendChild(
      svgEl("line", {
    ...
    if (total > 0) {
      svg.appendChild(
        svgEl("circle", {
          class: "ss-ld-dot",
          cx: X(total).toFixed(1),
          cy: Y(margins[total - 1]).toFixed(1),
          r: "5",
        }),
      );
    }
  ...
  function renderLedgerDiff(stage, vm) {
    ...
    if (vm.hasPointTypes) {
      ...
      cols = el("div", {
        class: "ss-ld-cols one",
        children: [buildLdGeneralCol(vm)],
      });
    }
    ...
    const body = el("div", {
      ...
      children: [
        buildLdDiffChart(vm),
        el("span", {
          class: "ss-ld-annot ss-ld-annot-top",
          text: `▲ ${t("home")}`,
        }),
        el("span", {
          class: "ss-ld-annot ss-ld-annot-bot",
          text: `▼ ${t("away")}`,
        }),
      ],
    });
    ...
    stage.appendChild(
      el("div", { class: "ss-ld-graph", children: [title, body] }),
    );
  }
    ...
    if (events.length === 0) {
      ...
      events.forEach((ev, idx) => {
        if (ev.kind === "point" && ev.team === teamNum) {
          ...
          track.appendChild(
            el("span", { class: "ss-chip ss-timeout", text: "T" }),
          );
        } else {
          track.appendChild(
            el("span", { class: "ss-chip ss-empty", text: "·" }),
          );
        }
      });
    }
  ...
  function pickRenderer(style) {
    switch (style) {
      case "brand_ledger":
        return renderBrandLedger;
      case "brand_columns":
        return renderBrandColumns;
      case "bento":
        return renderBento;
      case "glass":
        return renderGlass;
      case "ledger_diff":
        return renderLedgerDiff;
      case "bumper":
        return renderBumper;
      default:
        return renderBrandLedger;
  ...
  function renderSetSummary(state) {
    ...
    const style =
      typeof requested === "string" && KNOWN_VARIANTS.indexOf(requested) !== -1
        ? requested
        : "brand_ledger";
    ...
    const matchAnchorMs =
      vm.matchStartedAt && !vm.matchFinished
        ? Number(vm.matchStartedAt) * 1000
        : null;
    _liveTickState =
      setAnchorMs || matchAnchorMs ? { setAnchorMs, matchAnchorMs } : null;
    ensureLiveTick();
