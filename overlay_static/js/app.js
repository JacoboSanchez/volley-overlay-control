...
function applyStateLocale(state) {
  const next =
    state && state.raw_remote_customization
...
function runSideSwapTransition(view, raw) {
  ...
  const show = !!(
    view.overlay_control && view.overlay_control.show_main_scoreboard
  );
  ...
  if (scoreboard && scoreboard.dataset.fixedGeometry !== undefined) {
    ...
    if (panels.length) {
      ...
      gsap.to(panels, {
        xPercent: (i, el) => (el.classList.contains("pylon-away") ? 120 : -120),
        opacity: 0,
...
function renderSetPips(state) {
  ...
  [
    ...
    ["away-set-pips", state.team_away.sets_won],
  ].forEach(([id, won]) => {
    withEl(id, (el) => {
...
function relativeLuminance(rgb) {
  ...
  return (
    0.2126 * transform(rgb[0]) +
    0.7152 * transform(rgb[1]) +
    0.0722 * transform(rgb[2])
  );
}
...
function applyVerticalAnchor(state) {
  ...
  if (!anchor) {
    const cust = state && state.raw_remote_customization;
    const v =
      cust && typeof cust.verticalAnchor === "string"
...
function updateCSSVariables(teamHome, teamAway, colors = null) {
  ...
  if (teamHome.color_secondary)
    root.style.setProperty("--home-text", teamHome.color_secondary);
  root.style.setProperty("--away-primary", teamAway.color_primary);
  root.style.setProperty("--away-secondary", teamAway.color_secondary);
  if (teamAway.color_secondary)
    root.style.setProperty("--away-text", teamAway.color_secondary);
  ...
  root.style.setProperty(
    "--home-gradient",
    isNearWhite(teamHome.color_primary)
      ? teamHome.color_secondary || teamHome.color_primary
      : teamHome.color_primary,
  );
  root.style.setProperty(
    "--away-gradient",
    isNearWhite(teamAway.color_primary)
      ? teamAway.color_secondary || teamAway.color_primary
      : teamAway.color_primary,
  );
  ...
  root.style.setProperty(
    "--home-accent-dark",
    contrastAccent(teamHome.color_primary, true),
  );
  root.style.setProperty(
    "--away-accent-dark",
    contrastAccent(teamAway.color_primary, true),
  );
  root.style.setProperty(
    "--home-accent-light",
    contrastAccent(teamHome.color_primary, false),
  );
  root.style.setProperty(
    "--away-accent-light",
    contrastAccent(teamAway.color_primary, false),
  );
  ...
  if (colors) {
    ...
    if (colors.game_text)
      root.style.setProperty("--game-text", colors.game_text);
  }
}
...
function applyGeometry(container, geometry) {
  ...
  const scale =
    (Number(geometry.width) || BASE_WIDTH_PERCENTAGE) / BASE_WIDTH_PERCENTAGE;
...
function updateGeometry(geometry) {
  const container =
    document.getElementById("pill-wrapper") ||
...
function updateOutputTransform(geometry) {
  ...
  const marginFactor = Number.isFinite(marginPct)
    ? 1 - (2 * marginPct) / 100
    : 1;
...
function renderFullState(state, rawState) {
  ...
  // 4. Team Names & Logos
  withEl("home-name", (el) => {
    el.textContent = state.team_home.name;
  });
  withEl("away-name", (el) => {
    el.textContent = state.team_away.name;
  });
  equalizeNamePanels();
  fitBeachNames();
  ...
  // 5. Points & Sets
  withEl("home-points", (el) => {
    el.textContent = state.team_home.points;
  });
  withEl("away-points", (el) => {
    el.textContent = state.team_away.points;
  });
  withEl("home-sets", (el) => {
    el.textContent = state.team_home.sets_won;
  });
  withEl("away-sets", (el) => {
    el.textContent = state.team_away.sets_won;
  });

  // 6. Serving Indicator
  withEl("home-serving", (el) =>
    el.classList.toggle("active", state.team_home.serving),
  );
  withEl("away-serving", (el) =>
    el.classList.toggle("active", state.team_away.serving),
  );
  ...
  withEl("current-set-label", (el) => {
    el.textContent = setLabelText(state.match_info.current_set);
  });
  ...
  // 8. Bottom Ticker
  withEl("ticker-message", (el) => {
    el.textContent = state.overlay_control.ticker_message || "";
  });
...
function updateStateDiff(oldState, newState, oldRawState, newRawState) {
  ...
  if (
    oldState.overlay_control.preferredStyle !==
    newState.overlay_control.preferredStyle
  ) {
  ...
  // Visibility
  if (
    oldState.overlay_control.show_main_scoreboard !==
    newState.overlay_control.show_main_scoreboard
  ) {
    withEl("scoreboard-container", (container) => {
      applyScoreboardVisibility(
        container,
  ...
  // Geometry
  if (
    JSON.stringify(oldState.overlay_control.geometry) !==
    JSON.stringify(newState.overlay_control.geometry)
  ) {
  ...
  // Colors
  if (
    oldState.team_home.color_primary !== newState.team_home.color_primary ||
    ...
    JSON.stringify(oldState.overlay_control.colors) !==
      JSON.stringify(newState.overlay_control.colors)
  ) {
    updateCSSVariables(
      newState.team_home,
      newState.team_away,
      newState.overlay_control.colors,
    );
  }
  ...
  if (oldState.team_home.name !== newState.team_home.name) {
    withEl("home-name", (el) => {
      el.textContent = newState.team_home.name;
    });
  }
  if (oldState.team_away.name !== newState.team_away.name) {
    withEl("away-name", (el) => {
      el.textContent = newState.team_away.name;
    });
  }
  if (
    oldState.team_home.name !== newState.team_home.name ||
  ...
  if (
    oldState.overlay_control.show_logos !== newState.overlay_control.show_logos
  ) {
  ...
  if (oldState.team_home.serving !== newState.team_home.serving) {
    withEl("home-serving", (el) =>
      el.classList.toggle("active", newState.team_home.serving),
    );
  }
  if (oldState.team_away.serving !== newState.team_away.serving) {
    withEl("away-serving", (el) =>
      el.classList.toggle("active", newState.team_away.serving),
    );
  }
  ...
  // Set History
  if (
    oldState.match_info.current_set !== newState.match_info.current_set ||
    oldState.match_info.show_only_current_set !==
      newState.match_info.show_only_current_set ||
    JSON.stringify(oldState.team_home.set_history) !==
      JSON.stringify(newState.team_home.set_history) ||
    JSON.stringify(oldState.team_away.set_history) !==
      JSON.stringify(newState.team_away.set_history)
  ) {
  ...
  withEl("current-set-label", (el) => {
    el.textContent = setLabelText(newState.match_info.current_set);
  });
  ...
  if (
    ...
    oldState.match_info.set_summary_style !==
      newState.match_info.set_summary_style ||
  ...
  // 8. Ticker
  if (
    oldState.overlay_control.show_bottom_ticker !==
    newState.overlay_control.show_bottom_ticker
  ) {
    withEl("ticker-container", (tickerContainer) => {
  ...
  if (
    oldState.overlay_control.ticker_message !==
    newState.overlay_control.ticker_message
  ) {
  ...
  // 9. Player Stats
  if (
    oldState.overlay_control.show_player_stats !==
    newState.overlay_control.show_player_stats
  ) {
    withEl("player-stats-container", (statsContainer) => {
  ...
  if (
    JSON.stringify(oldState.overlay_control.player_stats_data) !==
    JSON.stringify(newState.overlay_control.player_stats_data)
  ) {
  ...
  if (
    ...
    JSON.stringify(oldOC.points_history) !==
      JSON.stringify(newOC.points_history)
  ) {
...
function equalizeNamePanels() {
  ...
  const selector = document.querySelector(".team-home .name-panel")
    ? ".name-panel"
    : ".team-info";
...
function renderSetHistory(state, isInitial = false) {
  ...
  if (state.match_info.show_only_current_set) {
    ...
    updateSetHistoryContainer(
      homeContainer,
      state.team_home.set_history,
      state.team_away.set_history,
      currentSet,
      bestOfSets,
      true,
    );
    ...
    if (!isCssDriven) {
      if (isInitial) {
        ...
        if (homeContainer.style.display === "none") {
          ...
          gsap.set(containers, {
            clearProps: "width,paddingLeft,paddingRight,opacity",
          });
...
function updateSetHistoryContainer(
  container,
  myHistory,
  theirHistory,
  currentSet,
  ...
  const cellWidth =
    parseFloat(getComputedStyle(container).getPropertyValue("--cell-w")) || 40;
...
function renderLiveStats(state) {
  ...
  const pcHome =
    (stats.partial_comeback || {})[1] ||
    (stats.partial_comeback || {})["1"] ||
    {};
  const pcAway =
    (stats.partial_comeback || {})[2] ||
    (stats.partial_comeback || {})["2"] ||
    {};

  if (cs.team && cs.n >= 2) {
    const teamClass =
      cs.team === 1 ? "live-stats-team-home" : "live-stats-team-away";
    _setStatsRow("live-stats-streak", "STREAK", cs.n + " in a row", teamClass);
  } else {
  ...
  if (ls.team && ls.n >= 4 && ls.n > (cs.n || 0)) {
    const teamClass =
      ls.team === 1 ? "live-stats-team-home" : "live-stats-team-away";
    _setStatsRow("live-stats-comeback", "LONGEST", ls.n + " streak", teamClass);
  } else {
    const peak = Math.max(pcHome.deficit || 0, pcAway.deficit || 0);
    if (peak >= 3) {
      const team = (pcHome.deficit || 0) >= (pcAway.deficit || 0) ? 1 : 2;
      const teamClass =
        team === 1 ? "live-stats-team-home" : "live-stats-team-away";
      _setStatsRow("live-stats-comeback", "COMEBACK", "-" + peak, teamClass);
    } else {
  ...
  if (typeof stats.total_points === "number" && stats.total_points > 0) {
    _setStatsRow("live-stats-totals", "POINTS", String(stats.total_points), "");
  } else {
...
function _matchFinished(state) {
  ...
  return (
    (state.team_home && state.team_home.sets_won >= threshold) ||
...
function _flashAlertOnTeam(team, className, durationMs) {
  ...
  targets.forEach((el) => {
    ...
    // Reading the layout property forces the browser to flush the reflow.
    void el.offsetWidth;
...
function dispatchAlertTransitions(oldState, newState) {
  ...
  if (
    oldState.team_home.sets_won !== newState.team_home.sets_won &&
    newState.team_home.sets_won > oldState.team_home.sets_won
  ) {
    triggerSetWonAlert("home");
  }
  if (
    oldState.team_away.sets_won !== newState.team_away.sets_won &&
  ...
  if (!wasFinished && nowFinished) {
    const winner =
      newState.team_home.sets_won > newState.team_away.sets_won
        ? "home"
        : "away";
