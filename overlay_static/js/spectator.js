...
(function () {
  ...
  try {
    document.title =
      (TRANSLATIONS[LANG] || TRANSLATIONS.en)["doc.title"] || document.title;
  } catch (_e) {
    /* ignore */
  }
  ...
  function setLogo(id, url) {
    ...
    const isImageData =
      proto === "data:" && parsed.pathname.toLowerCase().startsWith("image/");
  ...
  function resolveChartColors(homeColor, awayColor) {
    ...
    const fb =
      colorDistance(homeColor, COLOR_FALLBACK[0]) >=
  ...
  function renderTimers(broadcast) {
    ...
    const finishedAt =
      typeof match.match_finished_at === "number"
    ...
    if (typeof match.match_started_at === "number") {
      const reference =
        matchFinished && finishedAt != null ? finishedAt : Date.now() / 1000;
  ...
  function renderScoreboard(broadcast) {
    ...
    if (pipsHome)
      pipsHome.dataset.used = String(
        Math.max(0, Math.min(2, home.timeouts_taken || 0)),
      );
    if (pipsAway)
      pipsAway.dataset.used = String(
  ...
  function renderLastPoint(broadcast) {
    ...
    const team =
      lp.team === 1 ? broadcast.team_home || {} : broadcast.team_away || {};
  ...
  function renderSetChart(broadcast) {
    ...
    navLabel.textContent = t(isLive ? "chart.nav.setLive" : "chart.nav.set", {
      set: setNum,
    });
    ...
    const timeouts =
      timeoutsBySet[String(setNum)] || timeoutsBySet[setNum] || [];
    ...
    Array.from(
      svg.querySelectorAll(".set-chart-dot, .set-chart-timeout"),
    ...
    for (let i = 1; i < series.length; i += 1) {
      ...
      const x = projectX(p);
      // Home dot
      const cHome = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "circle",
      );
      ...
      // Away dot
      const cAway = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "circle",
      );
  ...
  function wireNavButtons() {
    ...
    if (next)
      next.addEventListener("click", () => {
        ...
        const best = Math.max(
          1,
          Math.min(7, (broadcast.match_info || {}).best_of_sets || 5),
        );
  ...
  function applySideSwap(broadcast) {
    ...
    if (mpi) {
      [
        ...
        ["team_1_match_point", "team_2_match_point"],
      ].forEach(([a, b]) => {
        const tmp = mpi[a];
        mpi[a] = mpi[b];
        mpi[b] = tmp;
